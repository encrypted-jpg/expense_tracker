...
def encrypt(key,message):
	# To Encrypt the credentials and data
    f = Fernet(key)
    encrypted = f.encrypt(message.encode())
    return encrypted

def generate():
	# For every piece of data a unique key is generated
    return Fernet.generate_key()
        
def decrypt(key,pass_hash):
    # To Decrypt the credentials and data
    f = Fernet(key)
...
def check(start_frame,user,passwd):
	...
	for users in users_list:
		if x1 == (decrypt(users["key"],(users["username"]))) and x2 == (decrypt(users["key"],users["password"])):
			Label(start_frame, text = "Logging in..").grid(row = 6, column = 1)
			try:
				start_frame.destroy()
			except :
				pass
			file_name = str(users["username"]) + ".pickle"
			storage = load()
			list_frame()
			k +=1
	user = None
...
def save_exp(add_e,exp,exp1):
	...
	try:
		...
		amt = encrypt(key, amount)
		x = encrypt(key, x)
		element = {"Time": x,"Name": name, "Amount": amt, "key": key}
		x = None
		x1 = None
...
def list_exp(flist):
	...
	total = 0
	for ele in storage:
		Label(list_e, text = decrypt(ele["key"],ele["Time"])).grid(row = y, column = 0)
		Label(list_e, text = "	").grid(row = y, column = 1)
		Label(list_e, text = decrypt(ele["key"],ele["Name"])).grid(row = y, column = 2)
		Label(list_e, text = "	").grid(row = y, column = 3)
		Label(list_e, text = decrypt(ele["key"],ele["Amount"])).grid(row = y,column = 4)
		y += 1
		total += float(decrypt(ele["key"],ele["Amount"]))
...
def show_graph():
	...
	dates = []
	for ele in storage:
		dt = (str(decrypt(ele["key"],ele["Time"])))[:8]
		if dt not in dates:
			dates.append(dt)
	totals = []
	for dts in dates:
		...
		for ele in storage:
			dt = (str(decrypt(ele["key"],ele["Time"])))[:8]
			if dt == dts:
				tot += float(decrypt(ele["key"],ele["Amount"]))
		totals.append(tot)
...
def load_users():
	# Load the users list from the users.pickle file
...
def add_user(create,ent,ent1):
	...
	global storage
	key = generate()
	user1 = encrypt(key,str(ent.get()))
	pass1 = encrypt(key,str(ent1.get()))
	users_list.append({"username": user1,"password":pass1,"key": key})
	pickle_out = open("users.pickle","wb")
	pickle.dump(users_list,pickle_out)
	pickle_out.close()
	name = str(user1) +".pickle"
	f = open(name,"wb")
